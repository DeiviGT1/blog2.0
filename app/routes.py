...
def dashboard():
    ...
    if request.method == "POST":
        ...
        if action == "generate_charts":
            ...
            # Generación de gráficos (ahora vía microservicio)
            if selected_country_bar:
                bar_image_base64 = get_plot_top_teams_country(df, selected_country_bar)

            if selected_country_rank:
                rank_image_base64 = get_plot_ranking_evolution(df, selected_country_rank)

            if show_violin:
                violin_image_base64 = get_plot_violin_elo(df)

            if show_heatmap:
                heatmap_image_base64 = get_plot_heatmap_elo(df)

            if selected_team_trend:
                team_trend_image_base64 = get_plot_team_trend_chart(df, selected_team_trend)
