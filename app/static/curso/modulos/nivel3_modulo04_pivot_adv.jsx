...
const SECTIONS = [{ title: "Calculated Fields", content: [{ type: "definition_list", items: [{ term: "Como crear", def: "PivotTable Analyze > Fields, Items and Sets > Calculated Field." }, { term: "Ejemplo", def: "Profit = Revenue - Cost. Aparece como campo en Values." }, { term: "Limitacion", def: "Operan sobre SUM, no sobre cada fila individual." }] }] }, { title: "Grouping", content: [{ type: "definition_list", items: [{ term: "Fechas", def: "Clic derecho > Group. Agrupa por Month, Quarter, Year." }, { term: "Numeros", def: "Agrupar en rangos automaticamente." }, { term: "Expandir/Colapsar", def: "Botones + y - para drill down." }] }] }, { title: "Slicers", content: [{ type: "definition_list", items: [{ term: "Que son", def: "Paneles de botones visuales. Insert > Slicer." }, { term: "Report Connections", def: "Un Slicer controla multiples PivotTables. Base de Dashboards." }, { term: "Seleccion multiple", def: "Ctrl+clic. Icono funnel para limpiar." }] }] }, { title: "Timelines", content: [{ type: "definition_list", items: [{ term: "Que son", def: "Barra deslizante para filtrar por periodo. Insert > Timeline." }, { term: "Requisito", def: "Solo campos de fecha reconocidos por Excel." }, { term: "Niveles", def: "Filtra por anos, trimestres, meses o dias." }] }] }, { title: "Show Values As", content: [{ type: "definition_list", items: [{ term: "% of Grand Total", def: "Porcentaje del total general." }, { term: "Running Total", def: "Acumulado progresivo." }, { term: "% Difference From", def: "Variacion porcentual vs un base." }] }] }, { title: "PivotCharts", content: [{ type: "text", value: "Graficos vinculados a PivotTables. PivotTable Analyze > PivotChart. Heredan Slicers y Timelines. Pieza central de Dashboards." }] }];
    ...
    <div style={{ maxWidth: 680, margin: "0 auto", padding: "0.5rem 0 2rem" }}>
      <div style={{ marginBottom: "1.5rem" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
          ...
          <span style={{ fontSize: 12, color: "var(--color-text-tertiary)" }}>Modulo 4 de 8</span>
        </div>
        <h1 style={{ fontSize: 22, fontWeight: 500, color: "var(--color-text-primary)", margin: "0 0 6px" }}>PivotTables avanzadas</h1>
        <p style={{ fontSize: 14, color: "var(--color-text-secondary)", margin: 0, lineHeight: 1.6 }}>Calculated Fields, Grouping, Slicers, Timelines. Reportes profesionales e interactivos.</p>
      </div>
      <div style={{ background: V[50], borderLeft: `3px solid ${V[400]}`, borderRadius: "0 var(--border-radius-md) var(--border-radius-md) 0", padding: "12px 16px", marginBottom: "1.5rem" }}>
        ...
        <p style={{ fontSize: 13, color: V[800], margin: 0, lineHeight: 1.6 }}>Que crees PivotTables avanzadas con campos calculados, agrupaciones y controles interactivos.</p>
      </div>
      ...
      <div style={{ background: "var(--color-background-secondary)", borderRadius: "var(--border-radius-lg)", padding: "1.25rem", marginTop: "1.5rem" }}>
        ...
        {["Calculated Fields crean metricas sin tocar datos fuente.", "Grouping agrupa fechas o numeros automaticamente.", "Slicers + Report Connections = base de Dashboards interactivos.", "Timelines filtran por periodo. Solo campos de fecha.", "Show Values As: porcentajes, acumulados, variaciones."].map((p, i) => (
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: "1.5rem", paddingTop: "1rem", borderTop: "0.5px solid var(--color-border-tertiary)" }}>
        <span style={{ fontSize: 13, fontWeight: 500, color: V[600] }}>Anterior: Formulas anidadas y Dynamic Arrays</span>
        <span style={{ fontSize: 13, fontWeight: 500, color: V[600] }}>Siguiente: Introduccion a Power Query</span>
      </div>
    </div>
  );
}
